...
void PlayScene::handleEvents()
{
	...
	if (SDL_NumJoysticks() < 1)
	{
		...
		else (EventManager::Instance().isKeyDown(SDL_SCANCODE_W))
...
void PlayScene::m_setGridLOS()
{
	...
	{
		m_pPatrolPath.push_back(m_pGrid[i * Config::COL_NUM * Config::ROW_NUM - 1 - i]); 
...
void PlayScene::m_movePlanetoTargetMode()
{
	if(m_bPatrolMode)
	
		m_pTargetPathNode = m_pPatrolPath[m_TargetPathNodeIndex];
	auto targetVector = Util::normalize(m_pTargetPathNode->getTransform()-> - m_pPlaneSprite->getTransform()->position);

	{
		
		auto targetVector = Util::normalize(m_pTargetPathNode->getTransform()->position - m_pPlaneSprite->()->position)
	...
	m_pPlaneSprite->getTransform()->position += m_pPlaneSprite->m_pPlaneSprite->getRigidBody()->velocity * m_pPlaneSprite->getRigidBody()->maxSpeed;
	if (m_pPlaneSprite->getTransform()->position == m_pTargetPathNode->getTransform()->position)

	{
		m_targetPathNodeIndex++; 
		if (m_targetPathNodeIndex > m_Patrol.size() - 1)
		{
